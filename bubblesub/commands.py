...
class SearchCommand(BaseCommand):
    ...
    def run(self, api):
        if not self.dialog:
            self.dialog = self.SearchDialog(api)
        self.dialog.exec_()
    ...
    class SearchDialog(QtWidgets.QDialog):
        def __init__(self, api, parent=None):
            ...
            self.find_next_btn = strip.addButton(
                'Find next', strip.ActionRole)
            self.find_prev_btn = strip.addButton(
                'Find previous', strip.ActionRole)
